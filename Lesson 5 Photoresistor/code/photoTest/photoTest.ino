...
void loop()
{
  ...
  ledLevel = map(sensorValue, 300, 650, 0, NbrLEDs); // map to the number of LEDs

  // turn on pins below level and off pins above level
  for (int led = 0; led < NbrLEDs; led++)
  {
    digitalWrite(ledPins[led], (led < ledLevel ? HIGH : LOW)); 
  }
}
